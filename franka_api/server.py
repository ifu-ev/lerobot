...
class FrankaAPI(franka_api_pb2_grpc.FrankaServiceServicer):
    def __init__(self):
        ...
        self.cart_pose_cmd_pub = rospy.Publisher("/equilibrium_pose", PoseStamped, queue_size=1)
    ...
    def SetCartPoseTarget(self, request, context):
        try:
            cmd = PoseStamped()
            cmd.pose.position.x = request.x
            cmd.pose.position.y = request.y
            cmd.pose.position.z = request.z

            cmd.pose.orientation.x = request.qx
            cmd.pose.orientation.y = request.qy
            cmd.pose.orientation.z = request.qz
            cmd.pose.orientation.w = request.qw
            
            rospy.loginfo(f"Publishing cart pose target: {cmd}")
            self.cart_pose_cmd_pub.publish(cmd)
            return franka_api_pb2.StatusResponse(success=True, message="Command sent")
        except Exception as e:
            return franka_api_pb2.StatusResponse(success=False, message=str(e))
