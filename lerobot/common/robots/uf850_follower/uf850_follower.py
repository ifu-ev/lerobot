...
class Uf850Robot(Robot):
    """
    uf850 robot integration for LeRobot.
    
    This class provides an interface to control the uf850 robot
    through a gRPC API connection.
    """

    config_class = Uf850Config
    name = "uf850_follower"

    def __init__(self, config: Uf850Config):
        super().__init__(config)
        self.config = config
        self.api = None
        self._connected = False
        self._calibrated = True  # uf850 is factory calibrated
    ...
    def is_calibrated(self) -> bool:
        """uf850 robots are factory calibrated."""
        return self._calibrated

    def calibrate(self) -> None:
        """uf850 robots don't require manual calibration."""
        logger.info(f"{self} is factory calibrated")
        self._calibrated = True

    def configure(self) -> None:
        """Configure robot parameters."""
        # uf850 configuration is handled by the gRPC server
